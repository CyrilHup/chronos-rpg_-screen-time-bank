...
const ActiveSession: React.FC<ActiveSessionProps> = ({ app, durationMinutes, onEndSession }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-zen-background flex flex-col items-center justify-center overflow-hidden">
        {/* Background Animation */}
        <div className="absolute inset-0 opacity-10">
            <div className="absolute top-0 left-0 w-full h-full bg-[linear-gradient(transparent_2px,white_2px),linear-gradient(90deg,transparent_2px,white_2px)] bg-[size:40px_40px] [mask-image:radial-gradient(circle_at_center,black,transparent)]" />
            <motion.div 
        ...
        {/* Top Bar HUD */}
        <div className="absolute top-0 left-0 w-full p-6 flex justify-between items-start text-zen-text-muted/50 font-mono text-xs z-10">
        ...
        <div className="relative z-10 w-full max-w-md p-8 text-center space-y-12">
            ...
            >
                ...
                <div className={`relative w-full h-full rounded-3xl ${app.color} flex items-center justify-center shadow-2xl border border-white/20`}>
                    ...
                    <span className="text-4xl font-bold text-zen-text-main">{app.name[0]}</span>
                </div>
            </motion.div>

            <div className="space-y-2">
                <h2 className="text-3xl font-bold text-zen-text-main tracking-tight">{app.name}</h2>
            ...
            <div className="relative">
                <div className="text-8xl font-mono font-bold text-zen-text-main tracking-tighter drop-shadow-[0_0_15px_rgba(255,255,255,0.3)]">
                    {formatTime(timeLeft)}
                </div>
                ...
                <svg className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-80 h-80 -rotate-90 pointer-events-none opacity-30">
                    <circle cx="160" cy="160" r="158" stroke="currentColor" strokeWidth="2" fill="none" className="text-gray-800" />
                    <circle 
                        cx="160" cy="160" r="158" 
